...
while(1){
  parsed_site <- remDr$getPageSource()[[1]]
  parsed_site <- read_html(parsed_site)
  next_page_t <- parsed_site %>% html_nodes("#panel div.panel_content_flexible > div.search_result .next")
  flag <- html_name(next_page_t)
  if(flag=="a"){
    # # next_page <- remDr$findElement(using = "css", "#dday_popup > div.popup_content.popup_link > button > span.img")
    # next_page <- remDr$findElement(using = "css", "#panel div.panel_content_flexible > div.search_result .next")
    # next_page$clickElement()
    # Sys.sleep(1)
    click_next_page(remDr)
...
# target_list<- rbind(page_list, target_list)
